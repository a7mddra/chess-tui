        ...
        return (
          <Text key={`rank-${rankLabel}`}>
            ...
            {row.map((cell, visualColIndex) => {
              const actualColIndex = isFlipped
                ? 7 - visualColIndex
                : visualColIndex;
              ...
              if (cell) {
                ...
                char = isPieceKind(kind)
                  ? getPieceGlyph(kind, cell.color)
                  : " ";
                if (
                  isValidMove &&
                  selectedPieceColor &&
                  cell.color !== selectedPieceColor
                ) {
              ...
              return (
                <Text key={`cell-${sq}`} backgroundColor={bg} color={color}>
                  {` ${char} `}
                </Text>
              );
            })}
